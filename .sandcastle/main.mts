...
// Phase-specific Codex model and effort settings
...
const sandboxProvider = docker({
  imageName: "sandcastle:mange-backend",
});

// Hooks run inside the sandbox before the agent starts each iteration.
// Prepare Go modules and frontend workspaces so agents can run project checks.
const hooks = {
  sandbox: {
    onSandboxReady: [
...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  const plan = await sandcastle.run({
    ...
    agent: sandcastle.codex(DEFAULT_CODEX_MODEL),
